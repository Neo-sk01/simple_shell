...
int builtin_cd(data_of_program *data)
{
	...
	if (data->token[1])
	{
		...
		{
			return (set_work_directory(data, data->token[1]));
		}
	}
	else
	{
...
int builtin_help(data_of_program *data)
{
	...
	for (i = 0; mensajes[i]; i++)
	{
		...
		length = str_length(data->token[1]);
		if (str_compare(data->token[1], mensajes[i], length))
		{
...
int builtin_alias(data_of_program *data)
{
	...
	{/* if there are arguments, set or print each env variable*/
		if (count_characters(data->token[i], "="))
